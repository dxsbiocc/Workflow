...
if [ -z "$env_exists" ]; then
    ...
    conda create -n snakemake python=3.10 snakemake=7.16 mamba -c bioconda -c conda-forge -y
else
...
echo "Append snakemake environment to bashrc ..."

snakemake_env=$(dirname $(which snakemake))

current_shell=$(echo $0 | awk -F'/' '{print $NF}')
if [ "$current_shell" == "bash" ]; then
    echo "export PATH=$snakemake_env:\$PATH" >> ~/.bashrc
elif [ "$current_shell" == "zsh" ]; then
    echo "export PATH=$snakemake_env:\$PATH" >> ~/.zshrc
fi
