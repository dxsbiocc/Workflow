...
USAGE="Usage: $0 [ -p sequence file Prefix ] [ -d database path] [ -l minimum match Length to filter. Default=44 ] [ -m minimum Match percentage to filter. Default=97]  [ -t number of Threads for blastn. Default=8 ] [ -o Outdirectory prefix Default=. ]"

unset name


while getopts ':p:d:l:m:t:o:cvh-' OPT
do
...
if [ -f ${outdir}.temp_file_list ]
then
	rm ${outdir}.temp_file_list
fi

if [ ${prefix} = "all" ]
then
        ls | grep "bam\|fq\|fastq" >${outdir}.temp_file_list

else
	if ls ${prefix}*.f*q* >/dev/null 2>&1
	then
		for x in `ls ${prefix}*.f*q*`; do echo $x >>${outdir}.temp_file_list; done
	fi

	if ls ${prefix}*.bam >/dev/null 2>&1
	then
		for x in `ls ${prefix}*.bam`; do echo $x >>${outdir}.temp_file_list; done
	fi
fi
...
reads_pref=$(for x in `cat ${outdir}.temp_file_list`; do ls $x | sed 's/\.fastq.gz//' | sed 's/\.fq.gz//' | sed 's/\.bam//' | sed 's/\.fastq//' | sed 's/\.fq//'; done)
...
for x in `cat ${outdir}.temp_file_list`; do echo $x; done
...
for file_path_prefix in `echo ${reads_pref}`
do
    x=$(basename "$file_path_prefix")
	...
	then
	   ...
	   then
		pigz -cd -p ${threads} ${read_path_str}/${x}.fastq.gz > ${outdir}/${x}.fastq 
	   else
	   	zcat ${read_path_str}/${x}.fastq.gz > ${outdir}/${x}.fastq 
	   fi

	   echo "Identifying reads in ${x}.fastq.gz with adapter contamination on $(date)."
	   cat ${outdir}/${x}.fastq | sed -n '1~4s/^@/>/p;2~4p' > ${outdir}/${x}.fasta
	...
	then
	   ...
	   if command -v pigz &> /dev/null
	   then
		pigz -cd -p ${threads} ${read_path_str}/${x}.fq.gz > ${outdir}/${x}.fq
	   else
	   	zcat ${read_path_str}/${x}.fq.gz > ${outdir}/${x}.fq 
	   fi

	   cat ${outdir}/${x}.fq | sed -n '1~4s/^@/>/p;2~4p' > ${outdir}/${x}.fasta 
	   blastn -db $DBpath/pacbio_vectors_db -query ${outdir}/${x}.fasta -num_threads ${threads} -task blastn -reward 1 -penalty -5 -gapopen 3 -gapextend 3 -dust no -soft_masking true -evalue 700 -searchsp 1750000000000 -outfmt 6 > ${outdir}/${x}.contaminant.blastout &
	   wait
	   ...
	   if command -v pigz &> /dev/null
	   then
	   	cat ${outdir}/${x}.fq | paste - - - - | grep -v -f ${outdir}/${x}.blocklist -F | tr "\t" "\n" | pigz -p ${threads} --fast > ${outdir}/${x}.filt.fastq.gz
	   else
	   	cat ${outdir}/${x}.fq | paste - - - - | grep -v -f ${outdir}/${x}.blocklist -F | tr "\t" "\n" | gzip -1 > ${outdir}/${x}.filt.fastq.gz
	   fi
	...
	then
	   ...
	   cat ${read_path_str}/${x}.fastq | sed -n '1~4s/^@/>/p;2~4p' > ${outdir}/${x}.fasta
	   blastn -db $DBpath/pacbio_vectors_db -query ${outdir}/${x}.fasta -num_threads ${threads} -task blastn -reward 1 -penalty -5 -gapopen 3 -gapextend 3 -dust no -soft_masking true -evalue 700 -searchsp 1750000000000 -outfmt 6 > ${outdir}/${x}.contaminant.blastout &
	   wait
	...
	then
	   ...
	   cat ${read_path_str}/${x}.fq | sed -n '1~4s/^@/>/p;2~4p' > ${outdir}/${x}.fasta
	   blastn -db $DBpath/pacbio_vectors_db -query ${outdir}/${x}.fasta -num_threads ${threads} -task blastn -reward 1 -penalty -5 -gapopen 3 -gapextend 3 -dust no -soft_masking true -evalue 700 -searchsp 1750000000000 -outfmt 6 > ${outdir}/${x}.contaminant.blastout &
	   wait
