...
nls_4peak <- function(x, y, k, estKmercov, estLength, max_iterations) {
  ...
  if (VERBOSE) {
    cat("trying nls_4peak standard algorithm\n")
  }

  try(model4 <- nls(
    y ~ (((2 * (1 - d) * (1 - (1 - r)^k)) + (2 * d * (1 - (1 - r)^k)^2) + (2 * d * ((1 - r)^k) * (1 - (1 - r)^k))) * dnbinom(x, size = kmercov / bias, mu = kmercov) * length +
  ...
  if (class(model4) == "try-error") {
    if (VERBOSE) {
      cat("retrying nls_4peak with port algorithm\n")
    }
...
score_model <- function(kmer_hist_orig, nls, round, foldername) {
  ...
  fit <- data.frame(
    all = model_fit_all, allscore = model_fit_allscore,
...
eval_model <- function(kmer_hist_orig, nls1, nls2, round, foldername) {
  ...
  if (!is.null(nls1)) {
    ...
    if (VERBOSE) {
      cat(paste("nls1score$all:\t", nls1score$all[[1]], "\n"))
    }
    ...
    if (VERBOSE) {
      cat("nls1score failed to converge\n")
    }
  }
  ...
  if (!is.null(nls2)) {
    ...
    if (VERBOSE) {
      cat(paste("nls2score$all:\t", nls2score$all[[1]], "\n"))
    }
    ...
    if (VERBOSE) {
      cat("nls2score failed to converge\n")
    }
  }
    ...
    if (!is.null(nls2)) {
      ...
      if (pdiff < SCORE_CLOSE) {
        ...
        if (het2 * SCORE_HET_FOLD_DIFFERENCE < het1) {
          if (VERBOSE) {
            cat(paste("returning nls1, similar score, higher het\n"))
          }
          return(list(nls1, nls1score))
        } else if (het1 * SCORE_HET_FOLD_DIFFERENCE < het2) {
          if (VERBOSE) {
            cat(paste("returning nls2, similar score, higher het\n"))
          }
      ...
      if (nls1score$all[[1]] < nls2score$all[[1]]) {
        if (VERBOSE) {
          cat(paste("returning nls1, better score\n"))
        }
        return(list(nls1, nls1score))
      } else {
        if (VERBOSE) {
          cat(paste("returning nls2, better score\n"))
        }
        return(list(nls2, nls2score))
      }
    } else {
      if (VERBOSE) {
        cat(paste("returning nls1, nls2 fail\n"))
      }
      return(list(nls1, nls1score))
    }
  }
  ...
  if (VERBOSE) {
    cat(paste("returning nls2 by default\n"))
  }
  return(list(nls2, nls2score))
}
...
estimate_Genome_4peak2 <- function(kmer_hist_orig, x, y, k, readlength, round, foldername) {
  ...
  if (VERBOSE) {
    cat(paste("trying with kmercov: ", estKmercov1, "\n"))
  }
  ...
  if (VERBOSE) {
    cat(paste("trying with kmercov: ", estKmercov2, "\n"))
  }
...
###############################################################################
wrapper <- function(snakemake) {
  ## Load the arguments from the user
  histfile <- snakemake@input[[1]]
  foldername <- snakemake@output[[1]]

  k <- as.integer(snakemake@params[["kmer"]])
  readlength <- as.integer(snakemake@params[["read_length"]])
  maxCovGenomeLen <- as.integer(snakemake@params[["kmer_max"]])

  VERBOSE <- as.logical(snakemake@params[["verbose"]])
  ...
  if (k > readlength) {
    stop("K cannot be greater than readlength")
  }
  ...
  if (kmer_prof[1, 1] == 0) {
    if (VERBOSE) {
      cat("Histogram starts with zero, reseting minkmerx\n")
    }
    minkmerx <- 2
  }
  ...
  if (VERBOSE) {
    cat(paste("using maxCovGenomeLen:", maxCovGenomeLen, " with index:", maxCovIndex, "trying 4peak model... \n"))
  }
  ...
  while (round < NUM_ROUNDS) {
    ...
    if (VERBOSE) {
      cat(paste("round", round, "trimming to", start, "trying 4peak model... \n"))
    }
      ...
      if (is.null(best_container[[1]])) {
        ...
        if (pdiff < SCORE_CLOSE) {
          ...
          if (hetb * SCORE_HET_FOLD_DIFFERENCE < hetm) {
            if (VERBOSE) {
              cat(paste("model has significantly higher heterozygosity but similar score, overruling"))
            }
            best_container <- model_4peaks
          } else if (hetm * SCORE_HET_FOLD_DIFFERENCE < hetb) {
            if (VERBOSE) {
              cat(paste("previous best has significantly higher heterozygosity and similar score, keeping"))
            }
          } else if (model_4peaks[[2]]$all[[1]] < best_container[[2]]$all[[1]]) {
            if (VERBOSE) {
              cat(paste("score is marginally better but het rate is not extremely different, upating"))
            }
            best_container <- model_4peaks
          }
        } else if (model_4peaks[[2]]$all[[1]] < best_container[[2]]$all[[1]]) {
