    ...
    def parser(self):
        ...
        if self.known:
            if isinstance(self.known, str):
                self.known = f"--known {self.known}"
            else:
                self.known = list(map("----known {}".format, self.known))

    def run(self):
        shell(
            "gatk3 {self.java_opts}"
            " --analysis_type RealignerTargetCreator"
            " --num_threads {self.snakemake.threads}"
            " --input_file {self.snakemake.input.bam}"
            " --reference_sequence {self.snakemake.input.ref}"
            " {self.known}"
            " {self.bed}"
            " {self.extra}"
            " --out {self.snakemake.output.intervals}"
            " {self.log}"
        )
