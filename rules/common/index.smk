if MAPPING == 'bwa':
    ...
    rule simpleaf_index:
        ...
        params:
            extra = "-r 90",
        log:
            opj(OUTDIR, "logs/mapped/simpleaf_index.log")
        wrapper:
            get_wrapper("simpleaf", "index")
elif MAPPING == 'cellranger':
    rule cellranger_index:
        ...
        log:
            opj(OUTDIR, "logs/mapped/cellranger_index.log"),
        wrapper:
            get_wrapper("cellranger", "mkref")
elif MAPPING == 'kbpython':
    if config['parameters']['kbpython']['mode'] == 'kite':
        rule kb_index:
            input:
                features = FEATURES,
            output:
            ...
            threads: 20
            params:
                extra = config['parameters']['kbpython']['extra'],
                mode = config['parameters']['kbpython']['mode'],
            log:
                opj(OUTDIR, "logs/mapped/kb_index.log")
            wrapper:
                get_wrapper("kallistobus", "ref")
    else:
        rule kb_index:
            input:
                fasta = REFERENCE,
