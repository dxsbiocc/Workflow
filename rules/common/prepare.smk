...
# --------------------- Control Flags ----------------------- #
# trimming tool
TRIMMING = config['control'].get('trimming', None)
# mapping tool
MAPPING = config['control']['mapping'].lower()
# deduplication tool
DEDUP = config['control'].get('dedup', None)
# --------------------- Reference Data ---------------------- #
# genome name
...
# scRNA kite mode
FEATURES = config['data'].get('features', None)
# ----------------------- Util rules ------------------------ #
include: "utils.smk"
...
if (not INDEX) or (not os.path.exists(INDEX)) or (os.listdir(os.path.dirname(INDEX)) == []):
    ...
    if MAPPING in ['star', 'minimap2', 'cellranger', 'simpleaf', 'kbpython']:
